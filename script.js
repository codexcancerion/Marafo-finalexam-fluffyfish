...
let shark_props = shark.getBoundingClientRect();
...
function play() {
    function move() {
        ...
        pipe_sprite.forEach((element) => {
            ...
            shark_props = shark.getBoundingClientRect();

            if (pipe_sprite_props.right <= 0) {
                element.remove();
            } else {
                if (shark_props.left < pipe_sprite_props.left + pipe_sprite_props.width && shark_props.left + shark_props.width > pipe_sprite_props.left && shark_props.top < pipe_sprite_props.top + pipe_sprite_props.height && shark_props.top + shark_props.height > pipe_sprite_props.top) {
                    img.style.display = 'none';
                    sound_die.play();
                    endGame();
                    return;
                } else {
                    if (pipe_sprite_props.right < shark_props.left && pipe_sprite_props.right + move_speed >= shark_props.left && element.increase_score == '1') {
    ...
    function apply_gravity() {
        ...
        if (shark_props.top <= 0 || shark_props.bottom >= background.bottom) {
            endGame();
            return;
        }
        shark.style.top = shark_props.top + shark_dy + 'px';
        shark_props = shark.getBoundingClientRect();
